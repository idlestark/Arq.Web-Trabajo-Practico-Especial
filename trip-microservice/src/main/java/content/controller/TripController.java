...
public class TripController {
    ...
    public ResponseEntity<Trip> createTrip(@RequestBody Trip trip) {
        ...
        return ResponseEntity.status(201).body(tripCreated);
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pause added successfully"),
            @ApiResponse(responseCode = "404", description = "Trip not found")
    })
    ...
    @Operation(summary = "Get scooters with most trips", description = "Gets scooter IDs with at least X trips in a year")
    @ApiResponse(responseCode = "200", description = "List of scooter IDs obtained successfully")
    @GetMapping("/scooter-with-most-trips")
    public ResponseEntity<List<Long>> getScootersWithMostTrips(
            @RequestParam int minTrips,
            @RequestParam int year) {
        List<Long> scooterIds = tripService.getScootersWithMinTrips(minTrips, year);
