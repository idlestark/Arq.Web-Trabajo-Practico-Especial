...
public class ScooterController {
    ...
    public ResponseEntity<Scooter> createScooter(@RequestBody ScooterDTO scooterDTO) {
        ...
        Scooter scooter = new Scooter(
                ...
                scooterDTO.getLatitude(),
                scooterDTO.getLongitude(),
                scooterDTO.getKilometers(),
                scooterDTO.getTimeUsed(),
                stop,
                scooterDTO.getBaseFee(),
                scooterDTO.getExtraFeePause()
        );
        Scooter newScooter = scooterService.saveScooter(scooter);
        if (newScooter == null) {
            return ResponseEntity.badRequest().build();
        }
        return ResponseEntity.status(201).body(newScooter);
    }

    @Operation(summary = "Delete scooter", description = "Deletes an existent scooter specified by its ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "Scooter deleted successfully"),
            @ApiResponse(responseCode = "404", description = "Scooter not found")
    })
    @DeleteMapping("/{id}")
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Scooter updated successfully"),
            @ApiResponse(responseCode = "404", description = "Scooter not found")
    })
    @PutMapping("/{id}")
    ...
    public ResponseEntity<Scooter> updateMaintenanceStatus(@PathVariable("id") Long id, @RequestParam("underMaintenance") boolean underMaintenance) {
        Scooter scooter = scooterService.findScooterById(id);
        if (scooter == null) {
            return ResponseEntity.notFound().build();
        }

        scooter.setUnderMaintenance(underMaintenance);
        scooterService.updateScooter(scooter);
        return ResponseEntity.ok(scooter);
    }
    ...
    public ResponseEntity<List<Scooter>> getScootersWithMostTrips(
            @PathVariable("minTrips") int minTrips,
    ...
    @Operation(summary = "Get nearby scooters", description = "Gets a list of all nearby scooters to specified coordinates")
    @ApiResponse(responseCode = "200", description = "Scooters list obtained successfully")
    @GetMapping("/nearby")
    public ResponseEntity<List<Scooter>> getNearbyScooters(
            @RequestParam double latitude,
            @RequestParam double longitude,
            @RequestParam(value = "radius", required = false) Double radius,
            @RequestParam(value = "ratio", required = false) Double ratio) {
        double r = radius != null ? radius : (ratio != null ? ratio : 5.0);
        List<Scooter> scooterList = scooterService.getNearbyScooters(latitude, longitude, r);
        return ResponseEntity.ok(scooterList);
    }
